...
export default function ManageStaff() {
  ...
  async function fetchStaff() {
    ...
    setLoading(true);
    const { data } = await supabase.from('profiles').select('*').eq('role', 'staff').eq('created_by', profile.id).order('created_at', { ascending: false });
    setStaff(data || []);
    setLoading(false);
  }
  ...
  async function toggleActive(member) {
    ...
    toast.success(`Account ${member.is_active ? 'deactivated' : 'activated'}`); fetchStaff();
  }

  async function deleteStaff(id) {
    if (!window.confirm('Delete this staff account?')) return;
    await supabase.from('profiles').delete().eq('id', id);
    toast.success('Staff deleted'); fetchStaff();
  }
